...
func TestValidateRequiredResults(t *testing.T) {
	...
	if err := validateRequiredResults([]string{"success", "success", "success", "success"}); err != nil {
	...
	for _, result := range []string{"failure", "cancelled", "skipped", ""} {
		...
		t.Run(result, func(t *testing.T) {
			...
			err := validateRequiredResults([]string{"success", "success", "success", result})
...
func TestValidateRequiredResultsRejectsMissingChild(t *testing.T) {
	...
	var failure *checkFailure
	err := validateRequiredResults([]string{"success", "success", "success"})
...
func TestRunRequiredExit(t *testing.T) {
	...
	if code := run([]string{"required", "success", "success", "success", "success"}, io.Discard, io.Discard); code != exitSuccess {
		t.Fatalf("run success code = %d", code)
	}
	var stderr strings.Builder
	if code := run([]string{"required", "success", "success", "success", "cancelled"}, io.Discard, &stderr); code != exitFailure {
