...
func run(args []string, stdout, stderr io.Writer) int {
	...
	if len(args) != 1 {
		_, _ = fmt.Fprintln(stderr, "usage: go run ./scripts/check <bootstrap|bootstrap-modules|foundation|platform|quick|security|security-history|vulnerability|fuzz|docs|freshness|qualification-freshness|format|release-gate|required RESULTS...>")
		return exitUsage
	}
