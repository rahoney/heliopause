...
func TestProbe(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing image", operatingSystem: "linux", executor: fakeExecutor{outputs: map[string]string{"docker version --format {{.Server.Version}}": "29.6.0", "runsc version": gVisorRelease, "docker info --format {{json .Runtimes.runsc}}": "{\"path\":\"/usr/local/bin/runsc\"}"}}, limitation: "M3_IMAGE_UNAVAILABLE"},
		{name: "available", operatingSystem: "linux", executor: fakeExecutor{outputs: map[string]string{"docker version --format {{.Server.Version}}": "29.6.0", "runsc version": gVisorRelease, "docker info --format {{json .Runtimes.runsc}}": "{\"path\":\"/usr/local/bin/runsc\"}", "docker image inspect " + nodeImageReference + " --format {{.Id}}": "sha256:example"}}, available: true},
	}
