...
func TestPythonSdistBuilderUsesOnlyVerifiedBuildWheels(t *testing.T) {
	...
	runner := &buildRunner{recordingRunner: recordingRunner{responses: [][]byte{[]byte("0123456789abcdef"), nil, nil, nil, nil, []byte("example-1.0-py3-none-any.whl\n"), nil}}, output: []byte("derived wheel")}
	...
	if len(runner.inputCalls) != 3 || len(runner.calls) != 7 {
