...
func TestProbePython(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing image", operatingSystem: "linux", architecture: "amd64", executor: fakeExecutor{outputs: map[string]string{"docker version --format {{.Server.Version}}": "29.8.1", "runsc --version": canonicalRunscVersionOutput, "runsc trace metadata": "Name: syscall/open_result\nName: sentry/mount_topology_snapshot\nName: sentry/mount_topology_mutation\n"}}, limitation: "M5_PYPI_IMAGE_UNAVAILABLE"},
