...
const (
	...
	canonicalRunscSidecarPolicyArg = "--sidecar-usage-policy=STRICT"
	canonicalRunscPodInitConfigArg = "--pod-init-config=/etc/heliopause/pod-init.json"
)
...
func (e *Executor) validateDaemon(ctx context.Context) error {
	...
	manifestIdentity, manifest, err := verifyLocalGVisorBundle(runtimeidentity.LocalGVisorBundleManifestPath, runtime.GOARCH, "")
	if err != nil {
		return fmt.Errorf("verify registered runsc-trace bundle: %w", err)
	}
	runsc, err := verifyExecutable(registeredPath, gVisorBundleMemberSHA512(manifest, "runsc"))
	if err != nil {
...
func parseRunscRegistration(body []byte) (string, error) {
	...
	if err := validateRunscRuntimeArgs(registered.RuntimeArgs); err != nil {
		return "", err
	}
	return registered.Path, nil
}

// validateRunscRuntimeArgs accepts only the runtime registration generated by
// Heliopause's runsc install invocation. Attached values and fixed ordering
// ensure that Go flag parsing treats STRICT as the sidecar policy itself; no
// argument can consume, override, or follow it. The workflow checker pins the
// corresponding install command independently.
func validateRunscRuntimeArgs(arguments []string) error {
	if len(arguments) != 2 || arguments[0] != canonicalRunscSidecarPolicyArg || arguments[1] != canonicalRunscPodInitConfigArg {
		return errors.New("runsc-trace registration does not use the canonical HAA runtime arguments")
	}
	return nil
}
...
func (e *Executor) tool(name string) (identity, error) {
	...
	if name == "runsc" {
		_, manifest, manifestErr := verifyLocalGVisorBundle(runtimeidentity.LocalGVisorBundleManifestPath, runtime.GOARCH, e.runscManifest.digest)
		if manifestErr != nil || gVisorBundleMemberSHA512(manifest, "runsc") != tool.digest {
			return identity{}, errors.New("local gVisor bundle changed after validation")
		}
	}
...
func verifyLocalGVisorBundle(path, goarch, expectedDigest string) (identity, runtimeidentity.LocalGVisorBundleManifest, error) {
	verified, err := verifyExecutable(path, "")
	if err != nil {
		return identity{}, runtimeidentity.LocalGVisorBundleManifest{}, err
	}
	file, err := os.Open(verified.path)
	if err != nil {
		return identity{}, runtimeidentity.LocalGVisorBundleManifest{}, errors.New("open local gVisor bundle manifest")
	}
	body, readErr := io.ReadAll(io.LimitReader(file, runtimeidentity.LocalGVisorBundleManifestSize+1))
	closeErr := file.Close()
	if readErr != nil || closeErr != nil || len(body) > runtimeidentity.LocalGVisorBundleManifestSize {
		return identity{}, runtimeidentity.LocalGVisorBundleManifest{}, errors.New("read bounded local gVisor bundle manifest")
	}
	current, err := os.Lstat(verified.path)
	if err != nil || !os.SameFile(verified.info, current) {
		return identity{}, runtimeidentity.LocalGVisorBundleManifest{}, errors.New("local gVisor bundle manifest changed while reading")
	}
	manifestDigest, manifest, err := validateLocalGVisorBundleManifestBody(body, goarch, expectedDigest)
	if err != nil {
		return identity{}, runtimeidentity.LocalGVisorBundleManifest{}, err
	}
	if err := verifyExactGVisorBundle(runtimeidentity.LocalGVisorBundleRoot, manifest); err != nil {
		return identity{}, runtimeidentity.LocalGVisorBundleManifest{}, err
	}
...
func validateLocalGVisorBundleManifestBody(body []byte, goarch, expectedDigest string) (string, runtimeidentity.LocalGVisorBundleManifest, error) {
	hash := sha512.Sum512(body)
	digest := hex.EncodeToString(hash[:])
	if expectedDigest != "" && digest != expectedDigest {
		return "", runtimeidentity.LocalGVisorBundleManifest{}, errors.New("local gVisor bundle manifest digest mismatch")
	}
	manifest, err := runtimeidentity.ParseLocalGVisorBundleManifest(body, goarch)
	if err != nil {
		return "", runtimeidentity.LocalGVisorBundleManifest{}, err
	}
	return digest, manifest, nil
}

func gVisorBundleMemberSHA512(manifest runtimeidentity.LocalGVisorBundleManifest, relativePath string) string {
	for _, member := range manifest.Members {
		if member.Path == relativePath {
			return member.SHA512
		}
	}
	return ""
}

func verifyExactGVisorBundle(root string, manifest runtimeidentity.LocalGVisorBundleManifest) error {
	if root != runtimeidentity.LocalGVisorBundleRoot || verifyNoSymlinkPath(root, false) != nil {
		return errors.New("local gVisor bundle root is unavailable")
	}
	rootInfo, err := os.Lstat(root)
	if err != nil || !rootInfo.IsDir() || rootInfo.Mode()&0o022 != 0 || verifyTrustedOwner(rootInfo) != nil || verifyTrustedParents(filepath.Dir(root)) != nil {
		return errors.New("local gVisor bundle root is untrusted")
	}
	expected := map[string]runtimeidentity.GVisorBundleMember{}
	for _, member := range manifest.Members {
	...
	err = filepath.WalkDir(root, func(candidate string, entry os.DirEntry, walkErr error) error {
		...
		if err != nil || (relative != "." && (filepath.IsAbs(relative) || filepath.Clean(relative) != relative)) {
			return errors.New("local gVisor bundle path is invalid")
		}
		if relative == "." || relative == "gvisor-bin" {
			if !entry.IsDir() {
				return errors.New("local gVisor bundle directory is invalid")
			}
			return nil
		}
		if entry.Type()&os.ModeSymlink != 0 || entry.IsDir() {
			return errors.New("local gVisor bundle contains an unexpected entry")
		}
		if relative == filepath.Base(runtimeidentity.LocalGVisorBundleManifestPath) {
			return nil
		}
		member, ok := expected[filepath.ToSlash(relative)]
		if !ok || seen[member.Path] {
			return errors.New("local gVisor bundle contains an unexpected file")
		}
		info, err := entry.Info()
		if err != nil || !info.Mode().IsRegular() || info.Size() != member.Size {
			return errors.New("local gVisor bundle member metadata mismatch")
		}
		if _, err := verifyExecutable(candidate, member.SHA512); err != nil {
			return errors.New("local gVisor bundle member identity mismatch")
		}
		seen[member.Path] = true
		return nil
	})
	if err != nil || len(seen) != len(expected) {
		return errors.New("local gVisor bundle is incomplete or invalid")
	}
	return nil
}
