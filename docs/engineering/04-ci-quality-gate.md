...
| Checkout Action | `actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1` | signed stable release `v7.0.1` |
...
- M0-006 최초 활성화에는 M0-002에서 고정한 checkout `v6.0.2`가 사용됐다. M0-008 qualification의 2026-08-13 KST 공급망 재검토에서 최신 signed stable `v7.0.1`과 v6.0.2 이후 unsafe PR checkout 방어·argument escaping 관련 수정이 확인되어, M0 완료 전에 full SHA를 `3d3c42e5aac5ba805825da76410c181273ba90b1`로 갱신했다.
- M0-007 활성화 시점인 2026-08-13 KST에도 Go `1.26.5`·`1.25.12`, `ubuntu-24.04`, `macos-26-intel`과 두 Action SHA가 lock과 일치함을 재확인했다. minimum Go와 macOS job은 quality tool을 설치하지 않고 `bootstrap-modules` 뒤 동일한 offline `platform` profile을 실행한다.
...
M0-007에서 활성 child는 `Quick`, `Docs`, `Minimum Go`, `macOS` 네 개다. `Required`는 네 `needs.*.result`가 모두 `success`인 경우에만 성공하며 결과 누락, `failure`, `cancelled`, `skipped`와 기타 값은 실패한다. Linux job은 Ubuntu build 결과만 증명하며 WSL2 qualification으로 표시하지 않는다.

## 7. Integration, E2E와 capability activation

M4-005에서 Promotion capability가 활성화되었다. `gVisor Linux Integration` controlled Linux job은 이미 pin한 Docker Engine과 Node image를 재사용하여 synthetic valid npm tarball을 target-local file lock으로 `--network none` offline install하고 exact output과 atomic publish를 검증한다. 이 test가 실행되지 않거나 image/runtime/lock/output validation이 실패하면 required integration job이 실패한다.
...
minimum Go와 macOS의 build/default test job은 external quality tool이 필요하지 않으므로 `bootstrap-modules`로 product module만 준비하고 `platform`을 offline 실행한다. Primary Quick은 `bootstrap`으로 Staticcheck까지 준비한다.
...
### M0-007 repository setting 점검과 적용 제안

2026-08-13 KST의 read-only API 확인 결과 repository는 public이고, Actions는 활성화되어 있지만 `allowed_actions=all`, `sha_pinning_required=false`였다. selected-actions 정책은 활성화되지 않았고 `main` branch protection과 repository ruleset도 없었다. repository-wide 변경에 대한 owner의 별도 적용 승인은 받지 않았으므로 M0-007에서는 설정을 변경하지 않았다.

적용 시에는 다음 범위로 제한한다.

- Actions를 `selected`로 전환하고 full SHA pinning requirement를 활성화한다.
- GitHub-owned 전체 또는 verified creator 전체를 포괄 허용하지 않고 현재 실제 consumer인 `actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1`, `actions/setup-go@b7ad1dad31e06c5925ef5d2fc7ad053ef454303e`만 exact pattern으로 허용한다.
- `main`은 pull request와 `Required` status check 성공, conversation resolution을 요구하고 force push와 deletion을 허용하지 않는다.
- required check는 성공 실행이 repository에 등록된 뒤 실제 GitHub Actions source와 함께 선택하며, workflow/job identity가 default branch에서 안정적으로 노출되는지 적용 직전에 다시 확인한다.

PR #1의 run `31663728891`에서 `Quick`, `Docs`, `Minimum Go`, `macOS`가 독립 실행되어 성공했고 `Required` aggregate도 성공했다. 이는 Heliopause CI gate 자체의 M0-007 검증 evidence이며 repository-wide setting 미적용은 M0 진행을 차단하지 않는다.
...
- [x] Step 12~14 유보 항목
# M3 gVisor observer helper CI

`gVisor Observer Helper` job은 Linux에서 exact gVisor source commit을 checkout하고 lock된 Bazel binary SHA-512를 확인한 후 `tools/gvisor-observer` overlay를 source tree에 복사해 Bazel build한다. 이 job은 product Go module과 독립이며 Go 1.25.12 minimum gate를 변경하지 않는다. helper build 또는 source/Bazel identity 검증 실패는 `Required` aggregate를 실패시킨다.

`gVisor Linux Integration` job은 GitHub runner에 사전 설치된 Docker version을 신뢰하지 않는다. Docker 공식 Ubuntu repository에서 Engine/CLI `29.6.2`와 그 요구 조건 `>= 2.1.5`를 만족하는 stable `containerd.io 2.3.3` exact package를 설치하고 실제 server version을 확인한 뒤에만 `runsc-trace`를 등록하고 lifecycle test를 실행한다. package 설치·version 확인·runtime 등록·observer 연결 중 하나라도 실패하면 required gate가 실패한다.

M4 resolver integration은 위 controlled CI daemon에 `firewall-backend=iptables`, `iptables=true`, `ip6tables=false`를 명시하고 restart 뒤 `DOCKER-USER` hook을 확인한다. 일부 Docker CLI/API 조합이 `docker info` backend field를 비워 두므로 실제 hook이 CI acceptance 기준이다. 이 설정은 GitHub runner의 기존 daemon configuration에 의존하지 않기 위한 test-environment pin이다. resolver helper는 제품 host의 daemon 설정을 바꾸지 않으며 backend/hook/rule apply·verify·cleanup을 확인할 수 없으면 fail-closed 한다.

M5-003은 같은 required Linux job에서 immutable Python image를 pull하고 exact
Bazel-built helper를 root-owned trusted installation path에 설치한 뒤,
process-scoped observer supervisor를 통해 실행한다. gated PyPI integration은 public
`packaging@25.0`의 wheel-only dry-run/report → Simple JSON cross-check를 default-deny
firewall lifecycle 안에서 수행한다. pinned image/helper/runtime, supervisor readiness,
observer stream, PyPI endpoint policy 또는 cleanup 중 하나라도 실패하면 integration
job과 Required aggregate가 실패한다.

M4 Promotion integration은 같은 exact Node image를 `--pull never`로 사용하고 test 중 network를 열지 않는다. fixture tarball은 test process가 local에서 deterministic하게 생성하며 public registry state를 success input으로 사용하지 않는다.

M5-006 PyPI Promotion integration은 같은 controlled Linux job에서 pinned Python image를 `--pull never`, `--network none`, read-only root와 non-root runtime으로 실행한다. fixture wheel은 test process가 deterministic하게 생성하며 public PyPI state를 success input으로 사용하지 않는다. pip의 offline/hash requirement install, installed `METADATA` identity, `RECORD` containment/hash와 atomic target publish 중 하나라도 실패하면 `gVisor Linux Integration`과 `Required` aggregate가 실패한다.

# M8 production trust requalification

M8 required CI는 runtime/observer lock JSON을 exact runtime identity의 canonical
owner로 사용한다. workflow shell, product Go constant와 workflow validator에 같은
commit/version/digest를 사람이 반복 입력하지 않는다. consumer가 generated
output을 사용하면 source marker, deterministic regeneration과 drift validator를
같은 required gate에 둔다.

Linux integration은 다음 production boundary를 실제 composition으로 검증한다.

- hostile PATH, ambient Docker/context/proxy environment와 remote endpoint rejection
- verified local Docker daemon endpoint와 registered runsc identity
- process-scoped observer supervisor start/readiness/exclusive ownership/death/cleanup
- connection container ID mismatch, dropped event와 unsupported observation
  fail-closed 처리
- one-time privileged network helper setup 뒤 ordinary-user `helox` resolver flow
- typed policy create/verify/remove와 arbitrary firewall argument rejection

unit fake만으로 위 boundary를 complete 처리하지 않는다. helper/service가 없거나
권한·endpoint·observation capability를 확인할 수 없으면 integration job은 skip
success가 아니라 failure다.

# M10 verified release and bootstrap gate

M10 release workflow는 ordinary CI와 분리하며 version/tag, source commit, release
signing identity, checksum, SBOM와 provenance/attestation subject를 하나의 release
manifest에 연결한다. signing identity의 owner, keyless issuer/repository/ref bound
또는 key custody, rotation, compromise/revocation 절차를 release 전 확정한다.

native `helox`, Linux observer helper와 실제 HAA-owned GHCR runtime image는 각각
digest와 provenance subject를 가진다. custom image는 pinned upstream base를 단순
