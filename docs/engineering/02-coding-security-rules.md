...
- Docker command는 ambient `DOCKER_HOST`, `DOCKER_CONTEXT`, credential/config와
  proxy를 상속하지 않는다. production configuration이 검증한 local daemon
  endpoint와 isolated client config만 사용하며 remote/context fallback을 금지한다.
- privileged Host helper는 raw executable/argument나 shell fragment를 받는 command
  proxy가 되어서는 안 된다. 권한별로 고정된 typed operation, bounded value,
  peer/session authorization과 cleanup contract를 정의한다.
...
- 여러 Adapter가 같은 Host tool을 사용하면 Composition Root가 검증한 concrete
  tool registry/executor를 주입한다. 각 Adapter가 독립적으로 PATH lookup이나
  ambient environment를 다시 구성하지 않는다.
