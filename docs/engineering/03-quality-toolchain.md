...
### M7-005 security tool identity lock

2026-08-21 KST M7-005 activation review locks the following tool identities.
They are quality tooling only: none enters the product module graph.
...
Go-installed tool which upstream intentionally labels `dev` is additionally
bound by its exact `package@version` installation and checksum verification;
Gitleaks release assets are bound by the published digest and `version` output.
Tool download, checksum, extraction, version mismatch or scanner failure is a
failed profile, never an unavailable-success fallback.

M7-005 adds one `.gitleaksignore` fingerprint only for historical commit
`aeb8fb81d531fd89ab67235245f3062de370a7d8`, where the product's own
redaction detector contained a private-key marker string rather than a key.
The current source splits that marker while preserving detection semantics. The
allowlist is commit, path, rule and line specific; it cannot hide a new finding.
...
- M0-005 활성화 시점인 2026-08-13 KST에 공식 release index와 `2026.1` release note를 다시 확인했으며, 최신 stable·Go 1.25/1.26 지원·binary identity가 M0-002 lock과 일치해 version 변경 없이 활성화한다.
...
`format`만 repository source를 변경할 수 있다. `bootstrap`과 `bootstrap-modules`는 source tree 밖 cache만 변경할 수 있으며, 나머지 check profile은 repository worktree에 대해 read-only여야 한다.
...
| `bootstrap-modules` | Yes | 현재 Go version으로 product module dependency만 source tree 밖 cache에 준비; quality tool을 build하지 않음 |
| `format` | No | tracked Go source에 `gofmt` 적용; 유일한 mutating profile |
| `platform` | No | 현재 Go·OS에서 production build와 default test; minimum Go와 macOS CI의 공통 계약 |
...
`quick + security + docs`는 `bootstrap`이 완료된 뒤 외부 service 없이 반복 가능한 기본 local 검증 집합이다. `platform`은 `bootstrap-modules` 뒤에 실행하며 quality tool의 setup Go와 minimum supported Go를 결합하지 않는다. `quick`의 formatting 단계는 항상 `gofmt -l` 기반 read-only format check이며 source를 수정하지 않는다. 이 profile들은 `GOTOOLCHAIN=local`과 offline module resolution을 사용해 toolchain이나 module을 암묵적으로 내려받지 않는다. cache miss는 자동 network fallback이 아니라 prerequisite failure다. `vulnerability`는 advisory database가 시간에 따라 변하고 network가 필요하므로 deterministic 검사와 분리한다. network failure를 취약점 없음으로 취급하지 않는다.
...
committed seed corpus는 sanitized·작고 재현 가능해야 한다. fuzzing은 명시적 time budget을 가진 별도 profile/scheduled job으로 실행하며 무제한 Quality Gate로 두지 않는다. M7 scheduled gate는 target마다 5초의 fuzzing budget과 cold compilation/teardown을 포함하는 1분 process deadline을 함께 적용한다. 발견된 crash input은 secret 여부를 확인한 뒤 regression corpus로 고정한다.
...
```text
gosec -include=G101,G107,G108,G109,G110,G111,G112,G113,G114,G201,G202,G203,G401,G402,G403,G404,G501,G502,G503,G504,G505,G601,G602 -nosec-require-rules -nosec-require-justification ./...
```

- production source와 repository-owned checker를 검사한다. test helper는
  Go compiler/test/race/fuzz gate로 검증하며, synthetic fixture path 진단을
  보안 finding으로 섞지 않는다.
...
- M7-005 is a fail-closed high-signal source-security gate: hard-coded secret,
  command/SQL/template injection, unsafe HTTP/TLS/crypto and blocklisted import
  rules run on every PR. `G104`, `G115`, `G122`, `G204`, `G301`, `G302`,
  `G304` and `G703` require context-sensitive root/size/command audits across
  pre-existing code, so they are reserved for M7-006's final security audit;
  they are not silently reported as a clean all-rules result.
...
# M3 gVisor observer helper boundary

M3 seccheck remote observer는 product Go module에 의존성으로 넣지 않는다. helper는 `tools/gvisor-observer` source overlay를 exact gVisor source commit 안에서 Bazel로 build하며, gVisor가 요구하는 Go/Bazel toolchain은 product minimum Go와 독립이다. product module은 Go `1.25.12` minimum gate를 유지한다.
